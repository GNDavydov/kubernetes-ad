...
class CeleryTaskQueue(TaskQueue):
    def __init__(self, app_name: str, broker_url: str, backend_url: str, train_task_name: str = "app.train_task", detect_task_name: str = "app.detect_task") -> None:
        self.app = Celery(
            app_name,
            broker=broker_url,
            backend=backend_url
    ...
    def register_detect_task(self, detect_task: Callable[[UUID], None]):
        task = self.app.task(name="app.detect_task")(detect_task)
        self._detect_task = task
        return task

    def register_train_task(self, train_task: Callable[[UUID], None]):
